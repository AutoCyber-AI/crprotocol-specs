...
**Title:** Context Relay Protocol (CRP) â€” Safety Policy Directive Language Specification  
**Version:** 3.0.0  
**Status:** Draft â€” IETF Internet-Draft Candidate  
...
; Source trust â€” which grounding sources are acceptable
...
; Halt â€” stop response delivery at specified risk level
halt-directive    = "halt-on" SP risk-level
risk-level        = "CRITICAL" / "HIGH" / "MEDIUM"

; Warn â€” pass response but flag at specified risk level
warn-directive    = "warn-on" SP risk-level

; Require â€” minimum quality/score thresholds
...
; Block â€” reject output containing specified content
...
; Quality â€” response quality requirements (NEW v3.0)
...
| `'none'` | No claims trusted â€” effectively blocks all AI output |
...
2. If `CRP-Safety-Hallucination-Risk` â‰¥ specified level â†’ HTTP 451 returned
...
**Enforcement:** If `CRP-Safety-Grounding-Pct` < threshold â†’ response rejected.

**Rejection behaviour:** If `upgrade-on-risk` is set, the gateway re-dispatches with `context-strict` grounding mode. If re-dispatch also fails â†’ HTTP 451.
...
**Enforcement:** If `CRP-Safety-Entailment-Score` < threshold â†’ response rejected (same flow as `require-grounding`).

### 3.6 require-quality (Minimum Quality Tier)

**Purpose:** Reject responses from envelopes below the specified quality tier.

**Enforcement:** If `CRP-Context-Quality-Tier` not in specified list â†’ HTTP 503.
...
### 3.7 require-flow (Minimum Flow Score) â˜… NEW

**Purpose:** Ensure multi-window responses maintain coherent flow.

**Enforcement:** If `CRP-Quality-Flow` < threshold â†’ re-dispatch with flow augmentation prompt (see CRP-SPEC-005 Â§11.5).
...
### 3.8 require-completeness (Minimum Completeness) â˜… NEW

**Purpose:** Ensure the response addresses all constituent information needs of the query.

**Enforcement:** If `CRP-Quality-Completeness` score < threshold â†’ auto-continuation window dispatched to cover uncovered sub-queries.
...
**Enforcement:** If `CRP-Compliance-GDPR-PII: true` â†’ response rejected. Especially important for public-facing AI systems where PII exposure is a GDPR Art. 5(1)(f) violation.

### 3.11 block-fabrication

**Purpose:** Block the response if any fabricated entity is detected by DPE Stage 3a.

**Enforcement:** If `CRP-Safety-Fabrications` > 0 â†’ response rejected. Strictest fabrication policy â€” used for medical, legal, financial domains.

### 3.12 block-repetition â˜… NEW

**Purpose:** Block the response if SEVERE repetition is detected by DPE Stage 7.

**Enforcement:** If `CRP-Quality-Repetition` level is `SEVERE` â†’ re-dispatch with anti-repetition prompt. If re-dispatch also produces SEVERE repetition â†’ halt.

### 3.13 max-repetition â˜… NEW
...
4. Reflexive dispatch includes verification pass â†’ lower risk expected
5. If re-dispatch still exceeds threshold â†’ halt (if `halt-on` set) or pass with HIGH warning
...
**Enforcement:** See CRP-SPEC-002 Â§5.10 (`CRP-Safety-Oversight-Mode`).
...
**Note:** `report-uri` for CRP-Safety-Policy naturally integrates with CRP Comply â€” the `audit_trail_uri` in the report links directly to the Comply evidence record.
...
halt-on CRITICAL + warn-on HIGH
  â†’ CRITICAL = halt, HIGH = warn, MEDIUM/LOW = pass

halt-on HIGH + warn-on MEDIUM
  â†’ HIGH/CRITICAL = halt, MEDIUM = warn, LOW = pass

halt-on CRITICAL + upgrade-on-risk reflexive
  â†’ HIGH = upgrade to reflexive and retry, CRITICAL = halt (even after upgrade)
```

### 4.2 CRP-Safety-Mode Override

`CRP-Safety-Mode` (see CRP-SPEC-002 Â§5.11) is a shorthand for common policy combinations:
...
All violations are computed and reported to `report-uri` but responses are never halted. This enables gradual policy rollout â€” observe violations before enforcing.
...
Child policy:   halt-on HIGH; require-grounding 0.80     â† VALID (tighter)
Child policy:   warn-on CRITICAL; require-grounding 0.50  â† INVALID (relaxed)
```
...
4. Any directive that is less restrictive â†’ reject with HTTP 403 and `CRP-Safety-Policy-Violation: inheritance`
...
CRP-Safety-Policy: profile=medical
  â†’ Expands to: default-src context; halt-on HIGH; require-grounding 0.90;
...
CRP-Safety-Policy: profile=financial
  â†’ Expands to: default-src context parametric; halt-on CRITICAL;
...
CRP-Safety-Policy: profile=developer
  â†’ Expands to: default-src context parametric; warn-on CRITICAL;
...
- `CRP-Safety-Nonce` (see CRP-SPEC-002 Â§5.16) binds the policy to a session nonce
- Gateway MUST validate policy syntax before accepting â€” malformed policies are rejected
